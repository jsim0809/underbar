(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    if(iterator === undefined) {
      // Simplest case, unsorted with no iterator
      var pushIfNotInResult = function(x) {
        if (_.indexOf(result,x) === -1) {
          result.push(x);
        }
      }
      _.each(array, pushIfNotInResult);
    } else {
      // With iterator
      var iteratedStuff = [];
      var pushIfNotInIteratedStuff = function(x) {
        if (_.indexOf(iteratedStuff,iterator(x)) === -1) {
          iteratedStuff.push(iterator(x));
          result.push(x);
        }
      }
      _.each(array, pushIfNotInIteratedStuff);
    }
    return result;
  };
  ...
  _.extend = function(obj) {
  	_.each(arguments, function(objectToBeAdded) {
  		for (var k in objectToBeAdded) { 
  			obj[k] = objectToBeAdded[k];
  		}
  	});
  	return obj;
  };
  ...
  _.memoize = function(func) {
  	...
  	var memoizedFunction = function() {
						      ...
						      if (storage[stringifiedArgs] === undefined) {
						        var result = func.apply(this, arguments);
						        storage[stringifiedArgs] = result;
						      }
  ...
  _.delay = function(func, wait) {
  	...
  	return setTimeout(execute, wait);
  };
  ...
  _.shuffle = function(array) {
  	...
  	while (copy.length > 0) {
  		...
  		randomized.push(copy[randomIndex]);
